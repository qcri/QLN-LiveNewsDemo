...
from feature_extraction.readability import Readability
...
class Functions:
    ...
    def fix(self, text):
        try:
            ...
            t = [unicodedata.normalize('NFKD', q) for q in text]
            text = ''.join(t).strip()
        return text

    def load_happiness_index_lexicon(self, filepath="feature_extraction/resources/"):
        word_to_happiness = {}
    ...
    def happiness_index_feats(self, text):
        ...
        with open("feature_extraction/resources/stopwords.txt") as stopdata:
    ...
    def load_moral_foundations_lexicon(self, filepath="feature_extraction/resources/"):
        code_to_foundation = {}
        foundation_to_lex = {}
        with open(os.path.join(filepath,
                               'moral foundations dictionary.dic')) as lex:
            ...
            for line in lex:
                try:
                    ...
                    for x in range(len(codes)):
                        foundation_to_lex[code_to_foundation[codes[x]]].append(
                                stem)
                except:
                    continue
    ...
    def moral_foundation_feats(self, text):
        ...
        for key in foundation_lex_dictionary.keys():
            foundation_counts[key] = float(sum([
                    stemed_tokens.count(i) for i in
                    foundation_lex_dictionary[key]])) / len(stemed_tokens)
        return foundation_counts["HarmVirtue"], \
            foundation_counts["HarmVice"], \
            foundation_counts["FairnessVirtue"], \
            foundation_counts["FairnessVice"], \
            foundation_counts["IngroupVirtue"], \
            foundation_counts["IngroupVice"], \
            foundation_counts["AuthorityVirtue"], \
            foundation_counts["AuthorityVice"], \
            foundation_counts["PurityVirtue"], \
            foundation_counts["PurityVice"], \
            foundation_counts["MoralityGeneral"]

    def load_acl13_lexicons(self, filepath="feature_extraction/resources/"):
            ...
            for line in lex:
                ...
                if line[0] == "type=weaksubj":
                    ...
                    if line[-1] == "priorpolarity=negative":
                        ...
                        spos.add(line[2].split("=")[1])
                        sneg.add(line[2].split("=")[1])
        return bias, assertives, factives, hedges, implicatives, report_verbs,\
            positive, negative, wneg, wpos, wneu, sneg, spos, sneu

    def bias_lexicon_feats(self, text):
        bias, assertives, factives, hedges, implicatives, \
            report_verbs, positive_op, negative_op, wneg, wpos, \
            wneu, sneg, spos, sneu = self.load_acl13_lexicons()
        tokens = word_tokenize(text)
        ...
        assertives_count = float(sum([
                tokens.count(a) for a in assertives])) / len(tokens)
        factives_count = float(sum([
                tokens.count(f) for f in factives])) / len(tokens)
        hedges_count = sum([tokens.count(h) for h in hedges]) + \
            sum([bigrams.count(h) for h in hedges]) + \
            sum([trigrams.count(h) for h in hedges])
        hedges_count = float(hedges_count) / len(tokens)
        implicatives_count = float(sum(
                [tokens.count(i) for i in implicatives])) / len(tokens)
        report_verbs_count = float(sum(
                [tokens.count(r) for r in report_verbs])) / len(tokens)
        positive_op_count = float(sum(
                [tokens.count(p) for p in positive_op])) / len(tokens)
        negative_op_count = float(sum(
                [tokens.count(n) for n in negative_op])) / len(tokens)
    ...
    def wordlen_and_stop(self, text):
        with open("feature_extraction/resources/stopwords.txt") as data:
    ...
    def stuff_LIWC_leftout(self, pid, text):
        ...
        try:
            ...
            print(pid)
        return (float(quotes) / len(tokens)) * 100, \
            (float(Exclaim) / len(tokens)) * 100, \
            (float(AllPunc) / len(tokens)) * 100, \
            allcaps

    def subjectivity(self, text):
        with open(os.path.join(DIRNAME, '', 'feature_extraction/resources/NB_Subj_Model.sav'),
                  'rb') as f:
            u = pickle.load(f)
            u.encoding = 'latin1'
            loaded_model = u
        with open(os.path.join(DIRNAME, '', 'feature_extraction/resources/count_vect.sav'),
                  'rb') as f:
            u = pickle.load(f)
            u.encoding = 'latin1'
            count_vect = u
        with open(os.path.join(DIRNAME, '', 'feature_extraction/resources/tfidf_transformer.sav'),
                  'rb') as f:
            u = pickle.load(f)
            u.encoding = 'latin1'
            tfidf_transformer = u
    ...
    def LIWC(self, text, cat_dict, stem_dict, counts_dict):
        ...
        counts_norm = [float(counts_dict[cat]) / WC * 100
